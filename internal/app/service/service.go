...
type ExportImport interface {
	...
	ExportXLS(xlsLink string, token string, client *http.Client, callback func(sheetName string, records [][]string) error) error
	ImporterXLS(credentials string, spreadsheetId string, sheetName string, records [][]string) error
}
